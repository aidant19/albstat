...
public class DBMatch {
    ...
    public ArrayList<DBSnapshot> getDBSnapshots(Match match) {
        ...
        for (Event event : match.events) {
            snapshots.add(new DBSnapshot(getMatchPlayerID(event.player1ID), 1, event.eventID, event.timestamp,
                    event.player1Snapshot));
            snapshots.add(new DBSnapshot(getMatchPlayerID(event.player2ID), 2, event.eventID, event.timestamp,
                    event.player2Snapshot));
            for (MainHandSnapshot groupSnap : event.group.snapshots) {
                snapshots.add(new DBSnapshot(getMatchPlayerID(groupSnap.playerID), 3, event.eventID, event.timestamp,
                        groupSnap));
            }
            for (ParticipantSnapshot partiSnap : event.participants.snapshots) {
                snapshots.add(new DBSnapshot(getMatchPlayerID(partiSnap.playerID), 4, event.eventID, event.timestamp,
                        partiSnap));
            }
        }
        return snapshots;
    ...
    public String toString() {
        return String.format("('%s', '%d', '%d', '%s', '%s')", match_id, (int) match_level, (int) match_winner,
